...
reg.register("furlong", UnitDef({"length": 1}, scale=201.168, canonical_name="furlongs"))
...
for name, est, is_divisor in chain.factors:
    print(f"{'/' if is_divisor else '*'} {name}: {est.value:.2g} {est._display_unit_str}")
...
from fermi_problems.formatting.notation import (
    to_scientific, to_engineering, to_human, to_order_of_magnitude
)
...
chain.add_factor("widgets_per_day", 500,  "widgets/factories")  # note: already registered
result = chain.compute()
```
...
from fermi_problems import (
    Unit, UnitRegistry, parse_unit, default_registry,
    Quantity, Estimate, EstimateChain, ChainResult, DimensionError,
    nearest_order_of_magnitude, order_of_magnitude, log10_distance,
    to_scientific, to_engineering, to_human,
    format_chain_result, format_estimate,
)
```
